...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for variable in eliminationOrder:
            currentFactors, joinedFactor = joinFactorsByVariable(currentFactors, variable)
            if len(joinedFactor.unconditionedVariables()) > 1:
                eliminated = eliminate(joinedFactor, variable)
                currentFactors.append(eliminated)

        jointCurrFact = joinFactors(currentFactors)
        normCurrentFact = normalize(jointCurrFact)

        return normCurrentFact
        "*** END YOUR CODE HERE ***"
