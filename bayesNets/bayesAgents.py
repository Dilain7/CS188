...
def constructBayesNet(gameState):
    ...
    # Iterate through the house and observation positions and save all
    # observation position in a list
    ...
    # Add tuples of edges where the first entry is the the starting point and
    # the second is the ending point of the edge
    ...
    [edges.append((FOOD_HOUSE_VAR, obs)) for obs in obsVars]
    [edges.append((GHOST_HOUSE_VAR, obs)) for obs in obsVars]

    # Add all the possible values that each variable could take
    variableDomainsDict[X_POS_VAR] = X_POS_VALS
    variableDomainsDict[Y_POS_VAR] = Y_POS_VALS
    for elem in HOUSE_VARS:
        variableDomainsDict[elem] = HOUSE_VALS
    for elem in obsVars:
        variableDomainsDict[elem] = OBS_VALS
...
def fillYCPT(bayesNet, gameState):
    ...
    # add the probabilities of the location relative to positions x and y
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Obtaining the resulting factors through variable elimination
    resultFactor = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)

    # Getting all the assignments of this new factor
    assignments = resultFactor.getAllPossibleAssignmentDicts()

    # Storing all the food house variable locations and resulting probabilities
    probList = []
    for assignment in assignments:
        probList.append([resultFactor.getProbability(assignment), assignment[FOOD_HOUSE_VAR]])

    # Choosing the location with the highest probability to be the food house
    temp = max(probList)
    prob, houseLoc = temp

    return({FOOD_HOUSE_VAR: houseLoc})
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        resultFactors = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        assignments = resultFactors.getAllPossibleAssignmentDicts()
        ...
        for assignment in assignments:
            if assignment['foodHouse'] == 'topLeft' and assignment['ghostHouse'] == 'topRight':
                pfLgR += resultFactors.getProbability(assignment)
            if assignment['foodHouse'] == 'topRight' and assignment['ghostHouse'] == 'topLeft':
                pfRgL += resultFactors.getProbability(assignment)

        leftExpectedValue = (pfLgR * WON_GAME_REWARD) + (pfRgL * GHOST_COLLISION_REWARD)
        rightExpectedValue = (pfRgL * WON_GAME_REWARD) + (pfLgR * GHOST_COLLISION_REWARD)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        evidenceExp = self.getExplorationProbsAndOutcomes(evidence)

        for evidence in evidenceExp:
            prob = evidence[0]
            exp = evidence[1]
            expectedValue += max(self.computeEnterValues(exp, enterEliminationOrder)) * prob
